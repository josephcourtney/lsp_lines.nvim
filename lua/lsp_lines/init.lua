...
local function render_current_line(diagnostics, ns, bufnr, opts)
    ...
    for _, diagnostic in pairs(diagnostics) do
        local show = diagnostic.end_lnum and (lnum >= diagnostic.lnum and lnum <= diagnostic.end_lnum) or
            ...
            if opts.virtual_lines.only_current_line then
                ...
                M.cache[bufnr] = {
                    scroll_offset = {
                        ...
                        y = vim.fn.winsaveview().topline, -- view
                    }
                }
                vim.api.nvim_create_autocmd({ "WinScrolled", }, {
                    callback = function()
                        ...
                        local x = vim.fn.winsaveview().leftcol
                        local y = vim.fn.winsaveview().topline

                        if (
                                (M.cache[bufnr].scroll_offset.x ~= x)
                                or (M.cache[bufnr].scroll_offset.y ~= y)
                            ) then
                            M.cache[bufnr].scroll_offset.x = x
                            M.cache[bufnr].scroll_offset.y = y
                            vim.diagnostic.hide(nil, bufnr)
                            vim.diagnostic.show(nil, bufnr, nil, opts)
                        end
                    end,
                    group = "LspLines"
                })
